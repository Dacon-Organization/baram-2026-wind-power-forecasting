...
**아래 표는 공개 후 재계산됐다.** 실제 label 범위 확인 결과 Group 3은 2023-01-01 01:00부터
시작하고 2022년 라벨이 0행이었다. 확정 정의는 [노트북 13](../../notebooks/13_seasonal_time_fold_lab.ipynb)
Decision Box ⑲이며, 아래가 그 결과다.

| Fold | Train | Valid | 목적 |
|------|-------|-------|------|
| F0 | 2022-01-01 ~ 2023-12-31 | 2024 전체 | 2025 유사 1년 holdout · 기존 기준선 |
| F1 | 2022-01-01 ~ 2023-12-31 | 2024-01-01 ~ 2024-05-31 | 겨울·봄 저온/강풍 |
| F2 | 2022-01-01 ~ 2023-12-31 | 2024-06-01 ~ 2024-10-31 | 여름·가을 장마/저풍속 |
| F3 | 2023-01-01 ~ 2023-12-31 | 2024 전체 | 2022 제거 영향 (Group 3 라벨 짧음 대응) |
| F4 | 2023-01-01 ~ 2024-06-30 | 2024-07-01 ~ 2024-12-31 | 최신 분포 rolling stability |

모든 fold가 `train 끝 < valid 시작`을 만족하며, 노트북 13이 이 조건과 겹침 0을
코드로 강제한다. 2025-01-01 00:00 경계 행 1건은 어느 fold에도 넣지 않는다.

**공개 전 정의에서 바뀐 점**

- F1·F2의 valid를 "겨울/봄", "여름/가을"이라는 서술에서 설계서 04의 월 구간(1-5월, 6-10월)으로 확정
- F3을 "2023 상반기 → 2023 하반기"에서 "2023 → 2024 전체"로 변경. 2024를 검증에 써야
  다른 fold와 같은 축에서 비교되고, 2022 제거 효과를 F0과 직접 대조할 수 있다
- F4의 train 시작을 2023-01-01로 두어 Group 3 라벨 구간과 맞췄다

**embargo gap:** 두지 않는다. `src/baram`에 `shift`·`rolling`·`expanding`·`ewm`·`diff`·`cum*`가
0건이며 모든 피처가 `forecast_kst_dtm` 단위로 계산되기 때문이다(노트북 13 Decision Box ⑳).
lag/rolling 피처를 도입하면 이 결정을 다시 연다.

### 6.3 Public 과적합 방지

**공개 후 확정.** Dacon 공식 안내문이 `Public Score : 전체 평가 데이터 중 사전 샘플링된 40%`,
`Private Score : 나머지 60%`, `1차 평가: 리더보드 Private Score(100%)`를 명시했다.
40:60은 이제 **공식 사실**이며, 이 문서의 공개 전 판단("공식 사실로 쓰지 않는다")을 대체한다.

다만 40%가 **어떤 단위로** 샘플링됐는지(시각 단위 무작위인지 일·주 블록인지)는 공개되지
않았다. 노트북 12는 시각 단위 무작위를 가정해 표본 잡음을 쟀으므로, 실제가 블록 표본이면
그 추정치는 하한이다. 계절 층화 표본이면 반대로 상한이다.

Public 점수에 맞춰 특정 계절/월 bias를 과하게 조정하지 않는 운영 원칙은 그대로 유지한다.
