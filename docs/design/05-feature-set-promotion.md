...
> 버전: v1.1
> 상태: 구현 완료 · G1~G5 통과 · 사람의 제출 버튼 대기
...
| 결과 (v1.1) | **G1~G5 전부 통과.** 채택 후보 2024 holdout total을 노트북 08과 절대 차이 0.0으로 재현. S1·S2 제출 후보와 ledger 초안 준비 완료 — 제출 버튼만 남음 |
...
**(v1.1 추가) 이 표의 값은 6자리 반올림 표기다.** 재현 판정에는 노트북 08 출력에 남은
전체 정밀도 값을 써야 한다. 6자리 반올림만으로 최대 5e-7 오차가 생겨 7.2절의 `1e-9`
게이트를 자동으로 넘기기 때문이다.

| 단계 | 전체 정밀도 | 출처 |
|------|------------|------|
| + wind vector (319) | 0.5864824798698839 | 08 셀 26의 `total_score − delta_vs_vector` |
| + turbine IDW p=2 (550) | **0.5924854297224146** | 08 셀 26 `total_score` |
| + turbine nearest own (396) | 0.5898283700164271 | 08 셀 26의 두 델타로 역산 |
| 공식 baseline (74) | — | 08이 전체 정밀도를 남기지 않아 6자리 대조만 가능 |
...
| G3 | 점수 재현 | `spatial_idw2_all_group`의 2024 holdout total score가 노트북 08의 `0.5924854297224146`과 일치 |
| G4 | 노트북 무결성 | `python scripts/check_notebook_integrity.py` 00~11 통과 |
...
**(v1.1 정정) 허용 오차만으로는 게이트가 성립하지 않는다. 비교 대상까지 명세해야 한다.**

| 항목 | 규정 |
|------|------|
| 기준값 | 1.1절의 6자리 표기가 아니라 **노트북 08 출력의 전체 정밀도 값** |
| 판정식 | `abs(재현값 − 전체정밀도값) <= 1e-9` |
| 전체 정밀도가 없는 경우 | `lab_official_mean`처럼 08이 남기지 않았으면 6자리 대조로 낮추고 그 한계를 결론에 명시 |
| 실측 결과 | 전체 정밀도 기준 3종 모두 **절대 차이 0.0** (2026-07-27, 노트북 11) |

6자리 표기를 기준으로 삼으면 정상 재현이 표기 오차(최대 5e-7)만으로 실패 판정되어,
멀쩡한 조립 계약을 의심하게 된다. 이는 G3의 목적과 정반대의 결과다.

### 7.2.1 (v1.1 추가) G3 재현 조건 체크리스트

노트북 08과 아래가 모두 같아야 점수가 재현된다. 하나라도 다르면 `max_features="sqrt"`인
RandomForest가 다른 트리를 만든다.

| 조건 | 값 |
|------|-----|
| 학습 구간 | 라벨 `year < 2024` — 17,519행 |
| holdout 구간 | 라벨 `year == 2024` — 8,784행 |
| 2025 경계 행 | 1건(2025-01-01 00:00)을 **어느 쪽에도 넣지 않음** |
| grid 통계 자유도 | `std_ddof=0` (`weather_grid.STD_DDOF` 기본값과 동일) |
| 공간 pooler fit 범위 | 2022~2023 raw 행에만 fit |
| imputer fit 범위 | 2022~2023 feature matrix에만 fit |
| 모델 파라미터 | `OFFICIAL_RF_PARAMS` 그대로 |
| 학습 mask | target별 non-null |
| clip | **예측에만** 0~capacity 적용. 실제값은 원본 유지 |

### 7.2.2 (v1.1 추가) 게이트 실측 결과 — 2026-07-27 KST

| ID | 결과 | 근거 |
|----|------|------|
| G1 | 통과 | `python -m pytest -q` → **237 passed** |
| G2 | 통과 | `official_mean` 파생 계약이 legacy 상수와 바이트 동일 (노트북 09 5-1절, `test_preprocessing_contract.py`) |
| G3 | **통과** | 전체 정밀도 기준 3종 절대 차이 **0.0**. 채택 후보 `0.5924854297224146` (노트북 11 3-2절) |
| G4 | 통과 | `check_notebook_integrity.py` → 00~11 **12개 통과**, missing_execution·error_outputs·consecutive_code_pairs 모두 0 |
| G5 | 통과 | S1·S2 validator 통과, run registry status `submission_validated`, submission SHA-256 기록 |

2024 holdout 점수 실측:

| feature_set | features | total | 1-NMAE | FICR |
|-------------|---------:|------:|-------:|-----:|
| `lab_official_mean` (랩 통제군) | 74 | 0.576938 | 0.863228 | 0.290647 |
| `official_mean` (production 기본값) | 76 | 0.577743 | 0.863317 | 0.292168 |
| `expanded_vector` | 319 | 0.586482 | 0.867895 | 0.305070 |
| **`spatial_idw2_all_group`** | **550** | **0.592485** | 0.869565 | 0.315406 |
| `spatial_nearest_own_group` | 396 | 0.589828 | 0.868767 | 0.310890 |

개선분 분해: 채택 후보의 총 +0.015548 중 1-NMAE 기여 +0.003169, FICR 기여 +0.012380 —
**FICR 기여율 79.6%**. 공식 산식이 계단 함수라 평균 오차 감소보다 정산 구간 진입이
점수에 크게 반영된다. 동시에 임계값 근처의 불안정성을 뜻하므로 local Δ가 Public에서
그대로 재현되지 않을 수 있는 첫 번째 이유다.
...
| S2 | `spatial_idw2_all_group` | 채택 후보 | S1 대비 Public Δ가 local Δ와 같은 방향인지 |

**(v1.1 정정) S1↔S2의 local Δ는 +0.015547이 아니라 +0.014742다.**
`+0.015547`은 랩 통제군(74개, lead 없음) 대비 값인데, S1이 실제로 쓰는 것은
production 기본값 `official_mean`(76개, lead 포함)이다. 두 기준선의 2024 holdout 점수가
다르므로 비교식도 달라진다.

| 기준선 | features | 2024 holdout total | S2와의 Δ |
|--------|---------:|-------------------:|---------:|
| 랩 통제군 `lab_official_mean` | 74 | 0.576938 | +0.015547 |
| **S1이 쓰는 `official_mean`** | **76** | **0.577743** | **+0.014742** |

Public Δ와 대조할 값은 아래쪽 행이다. lead feature 2개가 만든 +0.000805를 빼지 않으면
기울기 추정이 그만큼 낙관적으로 계산된다.
...
| # | 커밋 | SHA | 동작 변화 | 단독 revert |
|---|------|-----|-----------|-------------|
| 1 | `📝 Docs: 피처셋 승격과 첫 제출 파이프라인 설계서 추가` | `abf022f` | 없음 | 가능 |
| 2 | `✨ Feat: FeatureSetConfig와 프리셋 4종·YAML 오버라이드 추가` | `50ef870` | 없음 (호출부 없음) | 가능 |
| 3 | `✨ Feat: 터빈 메타데이터 로더 승격` | `d1ed1e5` | 없음 | 가능 |
| 4 | `✨ Feat: 피처 파이프라인 조립기 추가` | `ea068fd` | 없음 | 가능 |
| 5 | `♻️ Refactor: bundle에 target별 feature 컬럼 도입` | `682ac05` | 없음 (all-group 경로 동일) | 가능 |
| 6 | `✨ Feat: preprocessing contract 파생과 metadata schema 1.1` | `885f311` | hash 계산 경로 변경, 값은 동일 | 가능 |
| 7 | `✨ Feat: train/inference CLI 피처셋 연결` | `9b69bed` | **여기서 처음 동작 변화** | 가능 |
| 8 | `📝 Docs: 검증 노트북 09 피처셋 계약 랩 추가` | `af709de` | 없음 | 가능 |
| 9 | `📝 Docs: 검증 노트북 10 조립 파이프라인 랩 추가` | `d307655` | 없음 | 가능 |
| 10 | `📝 Docs: 검증 노트북 11 승격 스코어보드 추가와 G3 통과` | `0f52b15` | 없음 | 가능 |
| 11 | `📝 Docs: 설계서 05 v1.1 버전업과 대시보드 로그` | — | 없음 | — |
...
| 계획 | 실제 | 이유 |
|------|------|------|
| 커밋 1·2를 config와 YAML로 분리 | `50ef870` 하나로 합침 | YAML 해석기가 config 검증 규칙에 직접 의존해 분리 시 중간 커밋이 테스트를 통과하지 못한다 |
| 커밋 8을 golden 회귀 전용 테스트 커밋으로 | 각 구현 커밋에 테스트 동봉 | TDD RED→GREEN 순서상 테스트가 구현보다 먼저 들어가야 하며, 테스트만 있는 중간 상태를 남기지 않는 편이 revert 단위로도 깨끗하다 |
| 노트북 09·10·11을 한 커밋으로 | 세 커밋으로 분리 | 노트북 하나가 실행 실패해도 나머지를 되돌리지 않기 위해 |
...
| 단계 | 내용 | 산출물 | 상태 |
|------|------|--------|------|
| 1 | 계약 테스트 작성 (RED) | `tests/test_feature_config.py`, `tests/test_feature_pipeline.py`, `tests/test_turbine_metadata.py` | 완료 |
| 2 | config·YAML·로더 구현 (GREEN) | 커밋 2~3 | 완료 |
| 3 | 조립기·bundle 확장 구현 | 커밋 4~5 | 완료 |
| 4 | contract 파생·metadata 1.1 | 커밋 6 | 완료 |
| 5 | CLI 연결 | 커밋 7 | 완료 |
| 6 | G1~G2 게이트 실행 | 테스트 237건 통과 | 완료 |
| 7 | 검증 노트북 09·10·11 작성과 실행 | 커밋 8~10 | 완료 |
| 8 | G3~G5 게이트 실행 | 7.2.2절 실측 표 | 완료 |
| 9 | 제출 CSV 생성 + ledger 준비 | `outputs/submissions/` (미커밋) | 완료 |
| 10 | 대시보드 로그 + 설계서 버전업 | 커밋 11 | 완료 |
| 11 | **사람이 제출 버튼을 누르고 Public 점수 회수** | ledger의 `public_score`·`dacon_submission_id` | **대기** |
...
### 13.1 (v1.1) 구현 중 확정한 결정

| 날짜 | 결정 | 이유 |
|------|------|------|
| 2026-07-27 | G3 판정 기준을 6자리 표기가 아니라 노트북 08 출력의 **전체 정밀도 값**으로 명세 | 6자리 반올림만으로 최대 5e-7 오차가 생겨 `1e-9` 게이트를 자동으로 넘긴다. 정상 재현이 실패로 오판되면 멀쩡한 조립 계약을 의심하게 된다 |
| 2026-07-27 | 2025-01-01 00:00 경계 행 1건을 학습·holdout 어디에도 넣지 않음 | 공식 라벨이 01:00 시작 규약이라 이 행이 남는다. holdout에 넣으면 8,785행이 되어 노트북 08의 8,784행과 어긋나고 NMAE 분모가 바뀐다 |
| 2026-07-27 | S1↔S2 비교 기준선을 `official_mean`(76)으로 정정 | S1이 쓰는 것은 랩 통제군(74)이 아니다. local Δ는 +0.015547이 아니라 **+0.014742**이며, 이를 혼용하면 기울기 추정이 낙관적으로 계산된다 |
| 2026-07-27 | 제출 모델을 holdout 모델 재사용 대신 라벨 전체(26,304행)로 재학습 | 예측 대상 2025년과 가장 가까운 2024년을 버릴 이유가 없고, 라벨 마지막 시각이 제출 시간축 첫 시각보다 앞서 look-ahead가 없다. 대가는 제출 모델 자체가 검증된 적 없다는 것이며 ledger에 명시한다 |
| 2026-07-27 | 노트북 11에서 `official_mean`(76) 프리셋 점수를 별도 측정 | S1의 local 기준선을 정직하게 기록하기 위해. 결과 0.5777425681로 랩 통제군 대비 +0.000805 — lead feature 2개의 값이며 Decision Box ①의 선택이 점수 면에서도 손해가 아님을 확인 |
| 2026-07-27 | 제출물 생성을 노트북 내 파이프라인 직접 호출이 아니라 production CLI 경로로 수행 | `train.py`·`inference.py`가 실제로 동작하는지가 함께 검증되어야 한다. inference에 피처셋을 인자로 주지 않아 metadata 자동 복원 계약(6.2절)도 같이 확인된다 |
